...
RETURN = """
cpgs:
    description: returns the metadata of the 3PAR cpg(s)
    type:
        list: If no input name, returns the list of volumes
        dict: If input name is specified, returns dict of cpg object
        None: Returned in case of cpg not found
    sample: [
        {
            "name": "BL460_Bay6",
...
count:
    description: Size of the cpg response
    returned: always
    type: int
"""
...
try:
    ...
    #from hpe3par_sdk.models import Space
except ImportError:
    client = None
...
def getCPGInfo(cpg):
    ...
    #cpgInfo['SAGrowth']          = getGrowthParamJson(cpg.sagrowth)
    #cpgInfo['SDGrowth']          = getGrowthParamJson(cpg.sdgrowth)
    ...
    #cpgInfo['privateSpaceMiB']   = getPrivateSpaceJson(cpg.private_space_MiB)
...
def main():
    ...
    try:
        ...
        if not name:
            ...
            for cpg in dbCPGs:
                cpgs.append(getCPGInfo(cpg))
                count += 1
        else:
            dbCPG = client_obj.getCPG(name)
            cpgs.extend(getCPGInfo(dbCPG))
            count = 1
