...
def export_volume_to_hostset(
    ...
    try:
        ...
        if autolun:
            ...
            # Skip, if lun already present
            vluns = client_obj.getVLUNs()
            for vlun in vluns:
                if vlun.volume_name == volume_name and vlun.hostname == host_set_name:
                    return (True, False, "VLUN already present", {})
...
unexport a vlun',
        ...
        if lunid is not None:
            ...
            is_vlun_deleted = False
            for del_vlun in vluns:
                if del_vlun.volume_name == volume_set_name and del_vlun.hostname == host_name:
                    is_vlun_deleted = True
                    client_obj.deleteVLUN(volume_set_name, del_vlun.lun,
                                          host_name, port_pos)
                    break
            if not is_vlun_deleted:
                return (True, False, "VLUN not present in hostset %s (already deleted)" % host_name, {})
...
unexport a vlun',
        ...
        if lunid is not None:
            ...
            is_vlun_deleted = False
            for del_vlun in vluns:
                if del_vlun.volume_name == volume_set_name and del_vlun.hostname == host_set_name:
                    is_vlun_deleted = True
                    client_obj.deleteVLUN(volume_set_name, del_vlun.lun,
                                          host_set_name, port_pos)
                    break
